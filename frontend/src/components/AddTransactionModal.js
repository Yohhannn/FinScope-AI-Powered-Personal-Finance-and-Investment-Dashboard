    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${BASE_URL}/dashboard/transaction`, {
                ...
                body: JSON.stringify({ name, amount: parseFloat(amount), type, date, wallet_id: walletId, category_id: categoryId, description })
            });
            if(response.ok) {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Add New Transaction">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{type === 'expense' ? 'Budget / Category' : 'Source'}</label>
